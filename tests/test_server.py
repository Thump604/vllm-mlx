...
class TestSamplingDefaults:
    ...
    def test_extended_sampling_defaults_resolve_from_server_globals(self, monkeypatch):
        from vllm_mlx import server

        monkeypatch.setattr(server, "_default_top_k", 20)
        monkeypatch.setattr(server, "_default_min_p", 0.05)
        monkeypatch.setattr(server, "_default_presence_penalty", 1.5)
        monkeypatch.setattr(server, "_default_repetition_penalty", 1.1)
    ...
    def test_request_values_override_extended_sampling_defaults(self, monkeypatch):
        from vllm_mlx import server
