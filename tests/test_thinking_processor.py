...
class TestPhaseTransitions:
    ...
    def test_content_is_terminal_no_reentry(self):
        proc = _make_processor()
        completed = [10, 11, 50, 20, 21]
        _feed_sequence(proc, completed)  # reach CONTENT
        assert proc.state == Phase.CONTENT
        proc(mx.array(completed + [10]), _uniform_logits())
        proc(mx.array(completed + [10, 11]), _uniform_logits())
    ...
    def test_duplicate_prefix_does_not_double_count_thinking_tokens(self):
        proc = ThinkingAwareLogitsProcessor(
            start_token_ids=_START_IDS,
            end_token_ids=_END_IDS,
        ...
        proc(mx.array([101], dtype=mx.uint32), _uniform_logits())
        assert proc.state == Phase.THINKING
        assert proc.thinking_tokens == 1
    ...
    def test_prefix_rollback_restores_prior_state(self):
        proc = ThinkingAwareLogitsProcessor(
            start_token_ids=_START_IDS,
            end_token_ids=_END_IDS,
            thinking_token_budget=8,
...
class TestBudgetEnforcement:
    ...
    def test_transition_forces_end_sequence(self):
        ...
        # First transition token: the processor masks logits to force token 20,
        # but does not advance transition state until token 20 is observed.
        logits = proc(mx.array(tokens_so_far), _uniform_logits())
        # Only token 20 should have logit 0, rest -inf
        assert logits[20].item() == 0.0
        assert logits[0].item() == float("-inf")

        # Once token 20 is observed in the prefix, the next mask forces token 21.
        logits = proc(mx.array(tokens_so_far + [20]), _uniform_logits())
        assert logits[21].item() == 0.0

        proc(mx.array(tokens_so_far + [20, 21]), _uniform_logits())
        assert proc.state == Phase.CONTENT
    ...
    def test_forced_transition_tokens_not_counted(self):
        ...
        assert proc.thinking_tokens == 2
        # Feed through forced transition using the full prefix expected by the
        # state-synchronizing processor.
        proc(mx.array([10, 11, 50, 51, 20]), _uniform_logits())
        proc(mx.array([10, 11, 50, 51, 20, 21]), _uniform_logits())
...
class TestInnerDelegation:
    ...
    def test_inner_called_during_content(self):
        ...
        assert result[10].item() == float("-inf")
        assert result[20].item() == float("-inf")
        assert result[60].item() == (logits * 2)[60].item()
