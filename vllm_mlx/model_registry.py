...
class ServingProfile:
    """Per-model serving behavior beyond engine construction."""
...
def load_registry_config(
    ...
    for item in models:
        ...
        registry[name] = RegisteredModel(
            ...
            enable_auto_tool_choice=item.get("enable_auto_tool_choice"),
    ...
    async def _instantiate_model(
        ...
        if self._engine_factory is not None:
            ...
            engine = BatchedEngine(
                ...
                specprefill_draft_model_path=config.specprefill_draft_model,
    ...
    def _resolve_model_config(
        ...
        serving_profile = ServingProfile(
            enable_auto_tool_choice=(
                entry.enable_auto_tool_choice
                if entry.enable_auto_tool_choice is not None
                else self._defaults.serving_profile.enable_auto_tool_choice
            ),
            tool_call_parser=(
                entry.tool_call_parser
                if entry.tool_call_parser is not None
                else self._defaults.serving_profile.tool_call_parser
            ),
            reasoning_parser=(
                entry.reasoning_parser
                if entry.reasoning_parser is not None
                else self._defaults.serving_profile.reasoning_parser
            ),
            enable_thinking_default=(
                entry.enable_thinking_default
                if entry.enable_thinking_default is not None
                else self._defaults.serving_profile.enable_thinking_default
            ),
            force_nonempty_content=(
                entry.force_nonempty_content
                if entry.force_nonempty_content is not None
                else self._defaults.serving_profile.force_nonempty_content
            ),
        )
