...
class MLLMModelWrapper:
    ...
    def __init__(self, model):
        self._model = model
        # Detect if this is a Gemma 3/4 model (requires pixel_values as a
        # positional arg on text-only requests).
        self._is_gemma_multimodal = hasattr(model, "model_type") and any(
            marker in str(getattr(model, "model_type", "")).lower()
            for marker in ("gemma3", "gemma4")
        )
